...
RSpec.describe 'Teacher::Notifications#index', type: :system do
  ...
  before do
    ...
    visit teacher_lesson_reservation_notifications_path(lesson_reservation)
  end
  ...
  it 'テーブルボディに申し送りの内容と登録日時が表示されていること' do
    within 'tbody' do
      tds = all('td').map(&:text)
      expect(tds[0..1]).to eq [notification.body, notification.created_at.strftime('%Y/%m/%d %H:%M:%S')]
    end
  end

  it '新規登録をクリックすると新規登録画面へ画面遷移すること' do
    click_link '新規登録'
    expect(current_path).to eq new_teacher_lesson_reservation_notification_path(lesson_reservation)
  end

  it '編集をクリックすると編集画面へ画面遷移すること' do
    click_link '編集'
    expect(current_path).to eq edit_teacher_lesson_reservation_notification_path(lesson_reservation, notification)
  end

  it '削除をクリックすると申し送りが削除されること' do
    accept_confirm do
      click_link '削除'
    end
